...
class Book:
    ...
    def __init__(self, id_: int, name: str, pages: int) -> None:
        ...
        #if not isinstance(id, int): raise TypeError('id must be int')
        #if not isinstance(name, str): raise TypeError('name must be str')
        #if not isinstance(pages, int): raise TypeError('pages must be int')
...
class Library:
    ...
    def __init__(self, books=None) -> None:
        ...
        if not isinstance(books, list): self.books = list()
    ...
    def get_index_by_book_id(self, id) -> int:
        ...
        for i, j in enumerate(self.books, start=0):
